...
class CreatePost
{
    ...
    public function handle(Dispatcher $events, ThreadRepository $threads, PostValidator $validator, Censor $censor, Post $post)
    {
        ...
        if (!$isFirst) {
            $t = Post::query()->where('thread_id', $thread->id)->where('is_first', 1)->first();
            if ($this->actor->id != $aiuid && (in_array($thread->category_id,$arrcid) || ($callai && strpos($t->content, '@'.$aiusername) !== false))){
                $query = new ChatGptKernel();
                $query->toid = $thread->id;
                $query->type = 'TEXT';
                $query->msg_type = 1;
